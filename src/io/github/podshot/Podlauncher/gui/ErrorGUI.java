...
public class ErrorGUI extends JFrame implements ActionListener {
	...
	public ErrorGUI(Exception e) {
		...
		File log = new File("PodLauncher" + File.separator + "error_" + current.toString().replace(" ", "_").replace(":", "-") + ".log");
		try {
			log.createNewFile();
		...
		JLabel lblPleaseSeeThe = new JLabel("Please see " + log.getName() + " in the PodLauncher folder for");
